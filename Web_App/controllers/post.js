...
export function list() {
  return Post.find(
    {},
    { title: 1, themes: 1, producer: 1, views: 1, favs: 1, createdAt: 1 }
  )
    .sort({ createdAt: -1 })
    .exec()
}
...
export function filter(query, sort) {
  return Post.find(query).collation({ locale: 'en' }).sort(sort).exec()
}
...
export function deleteByProducer(producer) {
  return Post.deleteMany({ 'producer._id': producer }).exec()
}
...
export function addView(id) {
  return Post.updateOne({ _id: id }, { $inc: { views: 1 } }).exec()
}

export function getProducerById(id) {
  return Post.findOne({ _id: id }, { producer: 1 }).exec()
}

export function addFav(id) {
  return Post.updateOne({ _id: id }, { $inc: { favs: 1 } }).exec()
}
