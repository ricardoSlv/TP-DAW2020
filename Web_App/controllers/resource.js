...
export async function insert(user, resource, zipFile) {
  ...
  const newPath = join(
    ...
    newResource._id + '',
    '/',
    newResource._id + '.' + zipFile.originalname.split('.').pop()
  )
  const oldPath = join(__dirname, zipFile.path)
  try {
    // In case user directory doesnt exist
    if (!fs.existsSync(userDirectory)) await fsPromises.mkdir(userDirectory)
    ...
    await fs
      ...
      .pipe(unzipper.Extract({ path: userDirectory + '/' + newResource._id }))
      .promise()

    const jsonFile = await fsPromises.readFile(
      userDirectory + '/' + newResource._id + '/manifest.json'
    )
    const manifestData = JSON.parse(jsonFile)

    for (const file of manifestData.files) {
      await fsPromises.readFile(
        `${userDirectory}/${newResource._id}/${file.path}`
      )
      newResource.files.push(file)
    }
  } catch (e) {
    await fsPromises.rmdir(`${userDirectory}/${newResource._id}`, {
      recursive: true,
    })
    if (typeof e.path == 'string')
      throw new Error(
        ' Manifest error, could not find : ' +
          e.path.split('user_files/')[1].split('/').slice(2).join('/')
      )
    else throw e
  }
...
export async function deleteById(id) {
  try {
    ...
    const resourceDirectory = join(
      __dirname,
      'user_files/',
      resource.producer._id + '/' + resource._id
    )
...
export function deleteByProducer(producer) {
  return Resource.deleteMany({ 'producer._id': producer }).exec()
}

export function updateById(id, update) {
  return Resource.findByIdAndUpdate(id, update).exec()
}
...
export function filterByProducer(prodId, visibility) {
  const filter =
    visibility === undefined
      ? { 'producer._id': prodId }
      : visibility === true
      ? { 'producer._id': prodId, public: true }
...
export function filter(query, sort) {
  return Resource.find(query).collation({ locale: 'en' }).sort(sort).exec()
}

export function addDownload(id) {
  return Resource.updateOne({ _id: id }, { $inc: { downloads: 1 } }).exec()
}

export function getProducerById(id) {
  return Resource.findOne({ _id: id }, { producer: 1 }).exec()
}

export function addFav(id) {
  return Resource.updateOne({ _id: id }, { $inc: { favs: 1 } }).exec()
}
