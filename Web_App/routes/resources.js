...
router.get('/', async (req, res, _next) => {
  ...
  req.query.prodFilter &&
    (query['producer.name'] = new RegExp(req.query.prodFilter))

  const ordering = ['createdAt', 'registeredAt', 'downloads', 'favs'].includes(
    req.query.sortType
  )
    ? -1
    : 1
...
router.post('/upload', upload.single('zip'), async (req, res, _next) => {
  try {
    ...
    console.log(e)
    if (e.message === '409') res.sendStatus(409)
    else res.status(500).jsonp({ [e.name]: e.message })
  }
})

router.post('/edit/:id', async (req, res, _next) => {
  try {
    await Resource.editById(
      req.params.id,
      req.body.title,
      req.body.subtitle,
      req.body.type
  ...
  try {
    ...
    if (!resource)
      res.render('error', {
        ...
        error: {
          status: 404,
          stack: 'This resource does not exist or may have been deleted',
        },
      })
    else if (
      resource.public === false &&
      resource.producer._id.toString() != user._id.toString()
    ) {
      res.status(401)
      res.render('error', {
        user: req.user,
        error: { status: 401, stack: 'This resource has been set to private' },
      })
    } else res.render('resources/resource', { user, resource })
  } catch (e) {
    console.log(e)
    res.sendStatus(500)
  }
...
router.get('/:id/files/*', async (req, res, _next) => {
  ...
  if (req.user) {
    res.download(
      `./user_files/${resource.producer._id}/${resource._id}/${filepath}`
    )
  } else res.error(401, { user: req.user })
})
...
router.get('/edit/:id', async (req, res, _next) => {
  try {
    ...
    } else res.render('resources/edit', { user, resource })
  } catch (e) {
    console.log(e)
    if (e.message === '401') res.sendStatus(401)
...
router.get('/:id/download', async (req, res, _next) => {
  if (req.user) {
    ...
    res.download(
      `./user_files/${resource.producer._id.toString()}/${resource._id}/${
        resource._id
      }.zip`
    )
  } else {
    ...
    res.render('error', {
      ...
      error: {
        status: 401,
        stack: 'You must be logged in to download any resource',
      },
    })
  }
})
