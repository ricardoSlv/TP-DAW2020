...
router.get('/', async (req, res, _next) => {
  ...
  const ordering = ['createdAt', 'views', 'favs'].includes(req.query.sortType)
    ? -1
    : 1
  ...
  posts =
    req.query.sortType == 'themes'
      ? posts.sort((p1, p2) =>
          (p1.themes.sort()[0] || 'Z').localeCompare(p2.themes.sort()[0] || 'Z')
        )
      : posts
  ...
  try {
    ...
    if (!post)
      res.render('error', {
        ...
        error: {
          status: 404,
          stack: 'This post does not exist or may have been deleted',
        },
      })
    else {
...
router.get('/:id/resources', async (req, res, _next) => {
  try {
    ...
    else res.status(200).jsonp(post.resources)
  } catch (e) {
    console.log(e)
    if (e.message === '401') res.sendStatus(401)
    else res.sendStatus(500)
...
router.get('/edit/:id', async (req, res, _next) => {
  try {
    ...
    else res.render('posts/edit', { user, post, resources })
  } catch (e) {
    console.log(e)
    if (e.message === '401')
      res.render('error', {
        ...
        error: {
          status: 401,
          stack: 'You must be logged in to acess this post',
        },
      })
    else
      res.render('error', {
